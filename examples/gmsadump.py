...
from ldap3 import Connection, ALL, NTLM, SASL, GSSAPI, Server, Tls
...
class GetGMSAPasswords:
    ...
    def _resolve_sid(self, sid_canonical):
        ...
        try:
            success = self.__ldapConn.search(

                search_base=self.baseDN,
                search_filter='(objectSid={})'.format(sid_canonical),
                attributes=['sAMAccountName', 'name', 'cn', 'objectClass'],
            )

            if not success or not self.__ldapConn.entries:
                return sid_canonical
            entry = self.__ldapConn.entries[0]
    ...
    def processGMSAEntry_Ldap3(self, entry):
        sam = entry.sAMAccountName.value
        print(f"\n[*] Account: {sam}")

        # msDS-GroupMSAMembership is returned as raw bytes by ldap3
        if 'msDS-GroupMSAMembership' in entry:
            principals = self._parse_gmsa_acl(entry['msDS-GroupMSAMembership'].raw_values[0])
            print(f"    Readable by: {', '.join(principals)}")

        # msDS-ManagedPassword
        if 'msDS-ManagedPassword' in entry:
            pw_raw = entry['msDS-ManagedPassword'].raw_values[0]
            blob = MSDS_MANAGEDPASSWORD_BLOB()
            blob.fromString(pw_raw)
            ...
            print("    [-] msDS-ManagedPassword not retrieved (Check TLS/Permissions)")

    def ldap_auth(self) -> Connection:
        target_server = self.__kdcHost if self.__kdcIP else self.__domain
        self.__target = target_server
        dst_port = 636 if self.__useLdaps else 389
        tls_config = Tls(validate=ssl.CERT_NONE, version=ssl.PROTOCOL_TLSv1_2)
        server = Server(target_server, port=dst_port, get_info=ALL, use_ssl=self.__useLdaps, tls=tls_config)
        ldap_connection = partial(Connection, server, auto_bind=False)
        if self.__doKerberos:
            logging.debug("[*] Using SASL/GSSAPI (Kerberos)")
            return ldap_connection(authentication=SASL, sasl_mechanism=GSSAPI)
        else:
            logging.debug("[*] Using NTLM ")
    ...
    def run(self):
        ...
        try:
            self.__ldapConn.open()
        except Exception as e:
            logging.error("Bind failed: %s", e)
            exit(1)

        if self.__useLdaps:
            self.__tlsActive = True # Port 636 is already TLS
            logging.info('[+] Using LDAPS — Connection is secure.')
        elif not self.__doKerberos: # Port 389 + NTLM needs StartTLS
            try:
                self.__ldapConn.open()
                self.__ldapConn.start_tls()
                self.__tlsActive = True
                logging.info('[+] STARTTLS negotiated.')
            except Exception as exc:
                logging.warning('[-] STARTTLS failed: %s', exc)
        # if not self.__useLdaps and not self.__doKerberos:
        #     try:
        #         self.__ldapConn.start_tls()
        #         self.__tlsActive = True
        ...
        #         )
        #
        if not self.__ldapConn.bind():
            logging.error("BIND FAILED: %s", self.__ldapConn)
            exit(1)
        ...
        try:
            ...
            for entry in self.__ldapConn.entries:
                self.processGMSAEntry_Ldap3(entry)
